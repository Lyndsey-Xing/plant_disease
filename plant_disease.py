...
#路径
TRAIN_IMAGES_DIR = './data2/AgriculturalDisease_\
trainingset/images/'
VAL_IMAGES_DIR = './data2/AgriculturalDisease_\
validationset/images/'
TEST_IMAGES_DIR = './data2/\
AgriculturalDisease_testA/images/'
JSON_TRAIN = './data2/AgriculturalDisease_trai\
ningset/AgriculturalDisease_train_annotations.json'
JSON_VAL = './data2/AgriculturalDisease_\
validationset/AgriculturalDisease_validation_annotations.json'

log_dir = './log/'
model_dir = './data2/model/'
bottleneck_path = './data2/tmp/bottleneck/'
...
# inception_v3 路径
INCEPTION_DIR = './data2/model/inception_v3/tensorflow_inception_graph.pb'
...
def run_bottleneck_on_image(sess, image, image_data_tensor, bottleneck_tensor):
    ...
    rimage = tf.image.encode_jpeg(image.eval()).eval()
    bottleneck_values = sess.run(bottleneck_tensor,
                        {image_data_tensor: rimage})
...
def get_or_create_bottleneck(sess, image_name, image_path, category, 
    ...
    #如果特征向量文件不存在则通过Incepyion-v3来计算特征向量
    ...
    if not image_name+'.txt' in os.listdir(bottleneck_path+category+'/'):
        ...
        image = tf.gfile.GFile(image_dir, 'rb').read()
        ...
        # 可视化增强的图像
        #tensorflow在更新后，将函数名从tf.image_summary改为tf.summary.image
        #将参数名max_images改为max_outputs
        tf.summary.image('data_argument', image, max_outputs=9)
...
def get_batch_images(sess, json_dir, image_path, category, jpeg_data_tensor,
    ...
    for _ in range(BATCH_SIZE):
        ...
        # TODO: check the file name format: b'a2dfe1cf3298d95184499f9889a4620b.jpg'
        #print(image_name)
...
def main(_):
    # 加载inception-v3
    with tf.gfile.GFile(INCEPTION_DIR, 'rb') as f:
    ...
    with tf.Session() as sess:
        ...
        for step in range(STEPS):
            # 每次获取一个batch的训练数据
            print(sess, JSON_TRAIN, TRAIN_IMAGES_DIR, 'train',
                jpeg_data_tensor, bottleneck_tensor)
            ...
            if step%100 == 0 or step + 1 == STEPS:
                ...
                if cross_val_accuracy > max_acc:
                    ...
                    saver.save(sess, './data2/save/plant_disease.ckpt', global_step=step)
